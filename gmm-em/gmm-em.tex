...
\begin{document}
    ...
    \begin{figure}[t]
        \centering
            ...
            \hfill
            \includegraphics{px_pz_full.pdf}
            \hspace*{\fill}
        ...
        \begin{algorithmic}[1]
            ...
            \Function{EM}{$p_{Y,C}(y,c; \theta), p_{C|Y}(c|y; \theta), \theta^{(0)}$}
                \For{iteration $t \in 1,2,\ldots$}
                    \State{$q_C^{(t)} \gets p_{C|Y}(c|y; \theta^{(t-1)})$ (\textbf{E-step})}
                    \State{$\theta^{(t)} \gets \argmax_\theta \mathbb{E}_{q^{(t)}_C} \left[p_{Y,C}(y, C; \theta)\right]$(\textbf{M-step})}
